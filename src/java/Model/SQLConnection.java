...
public class SQLConnection {
    ...
    public static String QUERY_CREATEACCOUNT = "begin INSERT_USUARIO('%s','%s','%s','%s','%s','%s');end;"; // 
    ...
    public QueryResultEnum queryCreateUser(String username, String password, String tipoIdent, String numIdent) {
        String newPass = MD5Generator.GetMD5(password);
        String query = String.format(QUERY_CREATEACCOUNT, username, newPass, "USER", "OFFLINE", tipoIdent, numIdent);
        RowSet rSet = doQuery(query);
        if (rSet != null && rSet.success()) {
            return QueryResultEnum.SUCCESS; // Password correcto. Establecer conexión.
        } else {
