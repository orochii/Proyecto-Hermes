...
(
   ...
   FECHA_FINAL_CONTRATO DATE                
      constraint CKC_FECHA_FINAL_CONTR check (FECHA_FINAL_CONTRATO is null or (FECHA_FINAL_CONTRATO > FECHA_INICIO_CONTRATO)),
...
(
   ...
   PH_NUMERO_IDENTIFICACION NUMBER(20)          
      constraint CKC_PH_NUMERO_IDENTIF_CUENTA check (PH_NUMERO_IDENTIFICACION is null or (NUMERO_IDENTIFICACION > 0)),
   PH_TIPO_IDENTIFICACION VARCHAR2(10)        
      constraint CKC_PH_TIPO_IDENTIFIC_CUENTA check (PH_TIPO_IDENTIFICACION is null or (TIPO_IDENTIFICACION = 'EXT' OR TIPO_IDENTIFICACION = 'NAC' OR TIPO_IDENTIFICACION = 'JUR')),
   NOMBRE_CUENTA        VARCHAR2(50)         not null,
   TIPO_CODIGO_CUENTA   VARCHAR2(50)         not null,
   ULTIMA_FECHA_REAPERTURA DATE,
   FECHA_CREACION       DATE                 not null
      constraint CKC_FECHA_CREACION_CUENTA check (FECHA_CREACION <= SYSDATE),
   ...
   FECHA_EFECTIVA_CIERRE DATE                
      constraint CKC_FECHA_EFE_CIE_CUENTA check (FECHA_EFECTIVA_CIERRE is null or (FECHA_CREACION < FECHA_EFECTIVA_CIERRE)),
   FECHA_EFECTIVA_INICIO DATE                 not null
      constraint CKC_FECHA_EFE_INI_CUENTA check (FECHA_CREACION <=  FECHA_EFECTIVA_INICIO),
...
(
   ...
   FECHA_INGRESO        DATE                 not null
      constraint CKC_FECHA_INGRESO_INF_LAB check (FECHA_INGRESO <= SYSDATE),
...
(
   ...
   FECHA_EFECTIVA_FIN_PLAN DATE                
      constraint "CKC_FECHAS_PLAN " check (FECHA_EFECTIVA_FIN_PLAN is null or (FECHA_EFECTIVA_FIN_PLAN > FECHA_EFECTIVA_INICIO_PLAN)),
...
(
   ...
   FECHA_EFECTIVA_FIN_PRODUCTO DATE                
      constraint CKC_FECHA_EFECTIVA_FI_PRODUCTO check (FECHA_EFECTIVA_FIN_PRODUCTO is null or (FECHA_EFECTIVA_INICIO_PRODUCTO < FECHA_EFECTIVA_FIN_PRODUCTO)),
...
(
   ...
   FECHA_FIN_PROMOCION  DATE                
      constraint CKC_FECHA_FIN_PROM check (FECHA_FIN_PROMOCION is null or (FECHA_FIN_PROMOCION > FECHA_INICIO_PROMOCION)),
   FECHA_EFECTIVA_INICIO_PROMO DATE                 not null,
   FECHA_EFECTIVA_FIN_PROMO DATE                
      constraint CKC_FECHA_EFECTIVA_FI_PROMOCIO check (FECHA_EFECTIVA_FIN_PROMO is null or (FECHA_EFECTIVA_FIN_PROMO > FECHA_EFECTIVA_INICIO_PROMO)),
...
(
   ...
   FECHA_EFECTIVA_INICIO_SERV DATE                 not null
      constraint CKC_FECHA_EFECTIVA_IN_SERVICIO check (FECHA_EFECTIVA_INICIO_SERV < FECHA_EFECTIVA_FIN_SERV),
...
values(p_NUMERO_CONTRATO,p_CODIGO_CUENTA,p_NUMERO_IDENTIFICACION,p_TIPO_IDENTIFICACION,p_TIPO_CONTRATO,p_FECHA_INICIO_CONTRATO,p_NOMBRE_CONTRATO,p_CANAL_COMUNICACION_CLIENTE,p_DOCUMENTO_DIGITALIZADO,p_INDICADOR_RENOVACION_CONTRATO,p_NUMERO_ORDEN,p_TERMINOS_PAGO,p_CANAL_VENTA,p_RAZON_CANCELACION,p_FECHA_CANCELACION,p_FECHA_FINAL_CONTRATO,p_CODIGO_STATUS_CONTRATO,p_COSTO_CONTRATO,p_FECHA_INICIO_FACTURACION);
end;
/
...
values(p_CODIGO_PRODUCTO,p_CODIGO_PROMOCION,p_DESCUENTO,p_TIPO_PRODUCTO,p_DECRIPCION,p_NOMBRE,p_DEPOSITO_INICIAL,p_PRECIO_TOTAL,p_MONTO_CASTIGO,p_CODIGO_STATUS,p_FECHA_EFECTIVA_INICIO_PRODUCTO,p_FECHA_EFECTIVA_FIN_PRODUCTO);
end;
/
...
RETURN PDTypes.ref_cursor as
my_cursor PDTypes.ref_cursor;
begin
open my_cursor for
select * from CAMPANA
where CAMPANA.CODIGO_CAMPANA = p_CODIGO_CAMPANA;
...
begin
update PRODUCTO
set PRODUCTO.CODIGO_PROMOCION = p_CODIGO_PROMOCION, PRODUCTO.DESCUENTO = p_DESCUENTO, PRODUCTO.TIPO_PRODUCTO = p_TIPO_PRODUCTO, PRODUCTO.DECRIPCION = p_DECRIPCION, PRODUCTO.NOMBRE = p_NOMBRE, PRODUCTO.DEPOSITO_INICIAL = p_DEPOSITO_INICIAL, PRODUCTO.PRECIO_TOTAL = p_PRECIO_TOTAL, PRODUCTO.MONTO_CASTIGO = p_MONTO_CASTIGO, PRODUCTO.CODIGO_STATUS = p_CODIGO_STATUS, PRODUCTO.FECHA_EFECTIVA_INICIO_PRODUCTO = p_FECHA_EFECTIVA_INICIO_PRODUCTO, PRODUCTO.FECHA_EFECTIVA_FIN_PRODUCTO = p_FECHA_EFECTIVA_FIN_PRODUCTO
where (PRODUCTO.CODIGO_PRODUCTO = p_CODIGO_PRODUCTO);
end;
/
